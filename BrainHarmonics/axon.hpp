...
public:
    ...
    axon()
    {
    axon(std::chrono::high_resolution_clock::now(), 0);
    }
    
    axon(std::chrono::time_point<std::chrono::high_resolution_clock> eventTime, int val)
    {
    m_NeuronType = val;
...
#pragma omp task
    ...
    if (m_Energy > 0)
        ...
        for(std::vector<axonbouton>::iterator it = m_AxonBoutonList.begin(); it != m_AxonBoutonList.end(); ++it)
            {
            add_TemporalAdjustment(eventTime, &m_Energy, (0-m_Energy)/m_AxonBoutonList.size(), 100, 1);   // redistribute energy
            it->AddEnergy(eventTime, m_Energy/m_AxonBoutonList.size());
            }
        }
